...
import com.greazi.discordbotfoundation.mysql.query.action.Add;
import com.greazi.discordbotfoundation.mysql.query.action.Drop;
import com.greazi.discordbotfoundation.mysql.query.action.Modify;
...
public class Alter extends ExecuteStatement<Alter> {
	...
	public Alter(final ITable table) {
		super(table);
	}
	...
	public String toQuery() {
		final StringBuilder query = new StringBuilder("ALTER TABLE " + this.table + " ");
		...
		final List<String> dropColumns = drop.getColumns();

		for (int i = 0; i < columns.size(); i++)
			query.append(i != columns.size() - 1 ? columns.get(i).toString() + ", " : columns.get(i).toString() + (dropColumns.size() > 0 ? ", " : ";"));

		for (int i = 0; i < dropColumns.size(); i++)
			query.append(i != dropColumns.size() - 1 ? drop.getPrefix() + dropColumns.get(i) + ", " : drop.getPrefix() + dropColumns.get(i) + ";");
